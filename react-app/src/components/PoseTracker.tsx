...
//   const [blinkCount, setBlinkCount] = useState(0); // 디버그용 상태
//   const blinkCountRef = useRef(0);                 // 1초 단위 깜빡임 추적
...
//       blinkCountRef.current += 1;          // ✅ ref 증가
//       setBlinkCount((prev) => prev + 1);   // 디버그용
...
//       blink_count: blinkCountRef.current,  // ✅ 이번 1초 동안 감지된 깜빡임 수
//       gaze_x: irisX,
//       gaze_y: irisY,
//       head_pose: headPose,
...
//   const [analysisResult, setAnalysisResult] = useState("분석 대기 중");
//   const postureRef = useRef("정상");
//   const [postureState, setPostureState] = useState("정상"); // ✅ 자세 상태 useState
//   const [unstableSide, setUnstableSide] = useState<"left" | "right" | null>(null); // ✅ 추가
//   const earRef = useRef(0);
...
//     // postureRef.current = dy > threshold ? "불안정" : "정상";
//     // setPostureState(postureRef.current); // ✅ 자세 상태 업데이트
//     // console.log("📏 자세 상태:", postureRef.current);
//   if (Math.abs(dy) > threshold) {
//     const direction: "left" | "right" = dy > 0 ? "left" : "right";
//     postureRef.current = "불안정";
//     setPostureState("불안정");
//     setUnstableSide(direction); // ✅ 비동기 업데이트
//     console.log("📏 자세 상태: 불안정 | ↔ 방향:", direction); // ✅ 동기 출력
//   } else {
//     postureRef.current = "정상";
//     setPostureState("정상");
//     setUnstableSide(null);
...
//           {unstableSide === "left" && <div className="blinker left" />}
//           <p className="warning-text">자세가 불안정합니다!</p>
//           {unstableSide === "right" && <div className="blinker right" />}
//         </div>
//       )}
